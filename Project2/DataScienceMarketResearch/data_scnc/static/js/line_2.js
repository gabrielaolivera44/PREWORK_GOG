...
d3.csv("data_profiles.csv", function(error, data) {
  
  var sectorNames = d3.keys(data[0]).filter(function(key) { 
           return key !== "Profile"; });
  data.forEach(function(d) {
      d.profiles = sectorNames.map(function(name) { 
            return {name: name, value: +d[name]}; });
  });  
  x0.domain(data.map(function(d) { return d.Profile; }));
  x1.domain(sectorNames).rangeRoundBands([0, x0.rangeBand()]);
  y.domain([0, d3.max(data, function(d) { 
       return d3.max(d.profiles, function(d) { return d.value; }); })]);
  svg.append("g")
      .attr("class", "x axis")
...
var pointlabels = profile.selectAll(".pointlabels")
      .data(function(d) { return d.profiles; })
      .enter().append("g")
